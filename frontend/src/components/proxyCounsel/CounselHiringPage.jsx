...
/* Counsel browse-and-hire flow — shared by HireProxyCounsel.jsx
   ...
   should "feel identical" to requesting Proxy Counsel (single-appearance) —
   same browse grid, same filters, same select/negotiate/pay pipeline. There
   is no separate "full representation" professional pool in the schema, so
   both service types browse the exact same verified counsel pool
   (/public/proxy-counsels) and differ only in service_type on the hearing
   request that gets created and the presentation config pulled from
   ...
   still shared with NegotiationModule.jsx's counsel-profile fallback.
...
export default function CounselHiringPage({ serviceType }) {
